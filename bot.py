...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "confirm":
        ...
        await query.edit_message_text(
            "Please type the correct amount (numbers only, e.g. 1500):"
        )
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_id in pending_corrections and pending_corrections[chat_id].get("awaiting_amount"):
        try:
            new_amount = float(text.replace("$", "").replace(",", "").strip())
            pending_corrections[chat_id]["receipt"]["total"] = new_amount
            pending_corrections[chat_id]["awaiting_amount"] = False
            receipt = pending_corrections[chat_id]["receipt"]
            sign = "-" if receipt["type"] == "return" else "+"
            keyboard = [
                [InlineKeyboardButton("✅ Save now", callback_data="confirm")],
                [InlineKeyboardButton("✏️ Fix category", callback_data="fix_category")]
            ]
            await update.message.reply_text(
                "Amount updated to: " + sign + "$" + f"{new_amount:.2f}" + "\n\nReady to save?",
                parse_mode="Markdown",
                reply_markup=InlineKeyboardMarkup(keyboard)
            )
        except ValueError:
            await update.message.reply_text("Please enter just the number, e.g. 1500 or 85.50")
        return
